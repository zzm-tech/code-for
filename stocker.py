...
'''
TODO : 1.check data model
       2.ignore warning
'''


class Stocker():
    def __init__(self,ticker,year_1 = 2020,year_2 = 2024,mon_1 = 3,mon_2 = 3,days_1 = 13,days_2 = 13):
        '''
        version ： 2024 - 10 - 26
        python : 3.11
        bag : prophet
        ...
        self.days_1 = days_1
        self.days_2 = days_2
        start_date = datetime.datetime(year_1, mon_1, days_1)
        end_date = datetime.datetime(year_2, mon_2, days_2)

        # 获取股票数据
        ticker = yf.Ticker(ticker)
        stock = ticker.history(start=start_date, end=end_date)
        print(f"This is {ticker} stock, start date: {start_date}, end date: {end_date}")
        # print(f"describe {stock.head()}")

        # Set the index to a column called Date
        stock = stock.reset_index(level=0)
        print(f"describe {stock.columns}")
        ...
        self.starting_price = float(self.stock['Open'].iloc[0])

        # The most recent price
        self.most_recent_price = float(self.stock["y"][len(self.stock) - 1])
    ...
    def plot_stock(self, start_date=None, end_date=None, stats=['Close'], plot_type='basic'):
        ...
        for i, stat in enumerate(stats):
            ...
            print('Current {} = {:.2f} on {}.\n'.format(stat, self.stock[stat][len(self.stock) - 1],
                                                        self.max_date.date()))
    ...
    def buy_and_hold(self, start_date=None, end_date=None, nshares=1):
        ...
        # Find starting and ending price of stock and turn time
        ...
        start_price = float(self.stock.loc[self.stock['Date'] == start_date]['Open'])
        end_price = float(self.stock.loc[self.stock['Date'] == end_date]['Close'])
    ...
    def changepoint_prior_analysis(self, changepoint_priors=[0.001, 0.05, 0.1, 0.2], colors=['b', 'r', 'grey', 'gold']):
        ...
        train = self.stock[
<<<<<<< HEAD
            (self.stock['Date'] > (max(self.stock['Date']) - pd.DateOffset(years=self.training_years)))]
...
>>>>>>> remotes/origin/main
    ...
    def create_prophet_model(self, days=0, resample=False):
        ...
        stock_history['ds'] = stock_history['ds'].dt.tz_localize(None)
...
>>>>>>> remotes/origin/main
    ...
    def changepoint_date_analysis(self, search=None):
        ...
        if not search:
            ...
            print(c_data.loc[:, ['Date', 'Close', 'delta']][:5])
...
<<<<<<< HEAD
        self.stock['Date'] = self.stock['Date'].dt.tz_localize(None)
...
if __name__ == '__main__':
    # check code
    # stocker = Stocker('MSFT').plot_stock()
    # stocker = Stocker('MSFT').plot_stock(start_date = '2020-04-03', end_date = '2024-03-10',
    #                  stats = ['Daily Change', 'Volume'], plot_type='pct')

    # Stocker('MSFT').buy_and_hold(start_date = '2020-03-13', end_date = '2024-03-11', nshares=100)
    # Stocker('MSFT').buy_and_hold(start_date = '2020-03-13', end_date = '2024-03-11', nshares=100)

    # model, model_data = Stocker('MSFT').create_prophet_model()
    # model.plot_components(model_data)
    # plt.show()
    # print(Stocker('MSFT').weekly_seasonality)
    # Stocker('MSFT').weekly_seasonality = True
    # print(Stocker('MSFT').weekly_seasonality)
    # model, model_data = Stocker('MSFT').create_prophet_model()
    # model.plot_components(model_data)
    # plt.show()

    # Stocker('MSFT').weekly_seasonality = False
    # Stocker('MSFT').changepoint_date_analysis()
<<<<<<< HEAD
    # model, future = Stocker('MSFT').create_prophet_model(days=180)

    # Stocker('AMZN').evaluate_prediction()
    # Stocker('AMZN').changepoint_prior_analysis(changepoint_priors=[0.001, 0.05, 0.1, 0.2])
    Stocker('AMZN').changepoint_prior_validation(start_date = '2020-03-13', end_date = '2024-03-11',
